...
function conditionalP2(matrix,targetlabels,cutoff,min_region,min_cts,uscale,dscale,dolabel,doN_t)
...
%hold on;
%mask=signifP=="Few";
%[row_idx, col_idx] = find(mask == 1); % mask에서 값이 1인 위치를 찾음
%for i = 1:length(row_idx)
    %row = row_idx(i);
    %col = col_idx(i);
    %rectangle('Position', [col-0.5, row-0.5, 1, 1], 'FaceColor', 'black', 'EdgeColor', 'none'); 
%end
%hold off;
...
figure;imagesc(reorderedP);clim([0 1]);
set(gcf,"Position",[0 0 60*length(Plabels) 40*length(Plabels)]);
ax = gca;
ax.YTick=[];
ax.XTick=[];
...
save('contionalP.mat','conditionalp','conditionalp_u','matrix_binary','N_t','signifP','coE','coO','p_adjP','Plabels');