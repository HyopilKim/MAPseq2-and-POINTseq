function results=coinnervation_multiple(matrix,region_names,cutoff,colorscale,min_region,min_cts,pvaluecut,N_t)
...
matrix_binary_tmp=matrix_binary_tmp2(sum(matrix_binary_tmp2,2)~=0,:);
region_names=region_names(sum(matrix_binary_tmp,1)>=min_region);

matrix_filt=matrix_binary_tmp(sum(matrix_binary_tmp,2)~=0,:);


nTargets = size(matrix_filt,2);

    % 각 target의 hit 확률
    p = sum(matrix_filt, 1) / N_t;

    results = cell(nTargets, 1);
    ...
    for k = 2:nTargets
        ...
        for i = 1:nComb
            cols = combs(i,:);
            Obs(i) = sum(all(matrix_filt(:, cols) == 1, 2));
        end
    ...
    marktargets={};
    for i=2:nTargets
        regionidx_tmp=results{i}.RegionIdx;
...
for i=2:nTargets
    ...
    pre_obs=results{i}.Obs';
    pre_exp=results{i}.Exp';
    pre_relative=results{i}.overunder';
    pre_p_adjM=results{i}.pval_bonf';
    pre_signifM=results{i}.significance;
    ...
    linkaxes([ax1 ax2],'x');
    set(gca,'XTick',[],'YTick',[]);
    filename = sprintf('%d_targets.pdf', i);  % 01,02처럼 두 자리로 저장
