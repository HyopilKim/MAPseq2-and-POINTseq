function motif3(matrix,targetlabels,cutoff,cut_relativeobs,min_region,min_cts,maxtar,N_t,pvalue,doeachneuron)
...
mark_targets_filt=mark_targets(:,p_adjM<pvalue);
...
relative_filt=round(relative_obs(:,p_adjM<pvalue),2);
relative_filt_str=string(relative_filt);
relative_filt_str(relative_filt>100)=">100";

signifM_filt=signifM(p_adjM<pvalue);

neurons_mtf_filt=neurons_mtf(p_adjM<pvalue);
norm_neurons_mtf_filt=norm_neurons_mtf(p_adjM<pvalue);
neurons_mtfonly_filt=neurons_mtfonly(p_adjM<pvalue);
norm_neurons_mtfonly_filt=norm_neurons_mtfonly(p_adjM<pvalue);


%motif plot 
figure;set(gcf,'Units', 'normalized', 'Position', [0, 0,sum(p_adjM<pvalue)*0.03, 0.5]);
ax1=subplot(30,1,1:16);
obs_filt=obs(p_adjM<pvalue);
esti_filt=esti(p_adjM<pvalue);
bar(1:length(obs_filt),[obs_filt;esti_filt]);
motif_ylim=max([obs_filt esti_filt])*1.1;
set(gca,'XTick',[],'YLim',[0 motif_ylim],'FontSize',12);
for j=1:sum(p_adjM<pvalue)
...
plot_target_filt=plot_target_color(:,p_adjM<pvalue);
...
text(pre_text_x(p_adjM<pvalue),pre_text_y(p_adjM<pvalue),string(1:sum(p_adjM<pvalue)),'VerticalAlignment','bottom','HorizontalAlignment','center','FontSize',15);
hold off;
...
if exist('doeachneuron','var')
   ...
   % end
    ...
    %as matrix
   % targetfilt=plot_target(:,p_adjM<pvalue);
...
save('motif.mat','matrix_binary','obs','esti','p_adjM','signifM','relative_obs','norm_neurons_mtf_filt','norm_neurons_mtfonly_filt');