function motif_testtest(matrix,targetlabels,cutoff,cut_relativeobs,min_region,min_cts,p_adjP,maxtar,N_t,volcanoplotsize,volcanodotlabel,doeachneuron)
...
% 각 원소에 대해 binomial p-value 계산
for i = 1:length(pre_obs)
    if pre_obs(i) > pre_esti(i)
        pre_pvalueM(i) = 1 - binocdf(pre_obs(i) - 1, N_t, pre_esti(i)/N_t);
    else
        pre_pvalueM(i) = binocdf(pre_obs(i), N_t, pre_esti(i)/N_t);
    end
end
...
if min(pre_relative_obs)==0
    minrelobs=1/max(pre_relative_obs);
else
    minrelobs=0;
end
...
if volcanodotlabel==1
    if volcanoplotsize==0
        figure;scatter(relative_obs_volcano,p_volcano,50,'b', 'filled','MarkerFaceAlpha', 0.3);hold on;
...
save('motif_'+string(cutoff)+'.mat','matrix_binary','obs','esti','p_adjM','signifM','relative_obs','Pconnected_motif_asidx','Pconnected_motif_aslabel','norm_neurons_mtf_filt','norm_neurons_mtfonly_filt');