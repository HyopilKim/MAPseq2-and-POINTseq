...
[X, Y] = size(coO);
pvalueP = nan(X, Y);  % matrix to store results

for r = 1:X
    for c = 1:Y
        k_obs = coO(r, c);        % observed co-occurrence count
        p_null = coP(r, c);       % occurrence probability under the null model
        n = N_t;                  % total number of neurons (number of trials)
        ...
        % Two-tailed p-value: sum of probabilities less than or equal to the observed probability
...
save('conditionalP_'+string(cutoff)+'.mat','conditionalp','conditionalp_dice','sizefactor','matrix_binary','N_t','signifP','coE','coO','p_adjP','Plabels','numerator','divbyPb','rarity');