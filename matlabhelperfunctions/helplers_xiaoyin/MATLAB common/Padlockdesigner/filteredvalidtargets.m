...
%get the gene abbreviation for each query frmo the blast results
names=allnames(:,3);
padlocklist=validtargets(:,1);
for n=1:size(padlocklist)
...
for n=1:size(padlocks,1)
    ...
    for m=1:size(hits,1)
        ...
        if ~ismember(hitqueryname,hitsubname)
            names{n,2}=0;
            break
        end
    end
...
for i=1:length(uniqAccnum)
    ...
    elseif isempty(idx)
        finalist=[finalist;[allnames(idx,:) {} {}]];
    end
end
[~,I]=sort((finalist(:,2)));
finalist=finalist(I,:);
