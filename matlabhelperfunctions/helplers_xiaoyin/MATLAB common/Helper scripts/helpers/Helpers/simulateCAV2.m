...
function [output]=simulateCAV2(inputmatrix,inputnormalizedmatrix,infectionthreshold,injregions,regions,plotting)


density=zeros(length(injregions),length(regions));
for i=1:length(injregions)
    infected=inputmatrix(:,injregions(i))>=infectionthreshold;
