...
counts=readMat("your location/cts_distribution.mat")
...
proportion <- data.frame(source=c('VTA','VTA','VTA','VTA','VTA','SNc','SNc','SNc','SNc','SNc'),rbind(as.numeric(imported[[2]]),as.numeric(imported[[4]]),as.numeric(imported[[6]]),as.numeric(imported[[8]]),as.numeric(imported[[10]]),as.numeric(imported[[1]]),as.numeric(imported[[3]]),as.numeric(imported[[5]]),as.numeric(imported[[7]]),as.numeric(imported[[9]])))

colnames(proportion) <- c('source','1','2','3','4','5','>5')

long_data <- proportion %>%
  pivot_longer(cols = starts_with(c('1','2','3','4','5','>5')),
               names_to = "xpoint",
               values_to = "value")
...
ggplot(summary_data, aes(x = factor(xpoint,levels=c('1','2','3','4','5','>5')), y = mean, group = source, color = factor(source))) +
  geom_line(size=2) +
  geom_point() +
